...
uv sync
uv run dvc remote modify --local myS3storage access_key_id <key0>
uv run dvc remote modify --local myS3storage secret_access_key <key1>
uv run dvc push
dvc checkout

Чтобы узнать ключи <key0> и <key1> - писать в тг сюда @cqss0
